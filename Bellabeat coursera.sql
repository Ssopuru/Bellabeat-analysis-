...
select activitydate, sum(totalsteps) as steps_per_day, sum(totaldistance) distance_per_day, sum(sedentaryminutes) as sedentary_min_perday from dailyActivity_merged
group by 1;
...
/* user type based on total step per day
...
select id, (round(avg(totalsteps),2)) as avg_step, 
case
...
avg(totaltimeinbed - totalminutesasleep) as "avg_time_awake"
...
select id, avg(lightlyactiveminutes) as avg_lightly,
avg(fairlyactiveminutes) as avg_fairly,
avg(veryactiveminutes) as avg_very,
(avg(lightlyactiveminutes) + avg(fairlyactiveminutes) + avg(veryactiveminutes)) as avg_totalminutes,
